...
import {setSorted} from "../../../store/pizzaSlice.tsx";
...
const SortPopup: FC<ISortPopup> = ({}) => {
  ...
  const selectSort = useAppSelector(state => state.pizza.selectSort)
  ...
  const sortHandler = (e: React.MouseEvent<HTMLUListElement>) => {
    // @ts-ignore
    dispatch(setSorted(e.target.textContent))
     ...
     <div ref={sortRef} className="sort">
       ...
       {visiblePopup && <div className="sort__popup">
         <ul
            ...
            key={option}
            className={selectSort === option ? 'active' : ''}>{option}</li>)}</ul>
       </div>}
     </div>
  )
}
